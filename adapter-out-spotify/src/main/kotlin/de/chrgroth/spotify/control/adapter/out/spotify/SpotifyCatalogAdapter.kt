...
class SpotifyCatalogAdapter(
    ...
    override fun getArtist(userId: UserId, accessToken: AccessToken, artistId: String): Either<DomainError, AppArtist?> {
        return try {
    ...
    override fun getTrack(userId: UserId, accessToken: AccessToken, trackId: String): Either<DomainError, String?> {
        return try {
            throttler.throttle(DomainOutboxPartition.ToSpotify.key)
            val request = HttpRequest.newBuilder()
    ...
    override fun getAlbum(userId: UserId, accessToken: AccessToken, albumId: String): Either<DomainError, AppAlbum?> {
        return try {
            throttler.throttle(DomainOutboxPartition.ToSpotify.key)
            val request = HttpRequest.newBuilder()
