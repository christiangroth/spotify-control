...
## New Features
* Added two checks for "End of the Road": one flags artists that should be included but have no track there, the other flags artists that are excluded but still have one.
* An artist with a track on "End of the Road" now stays fully synced; an artist newly added only to "Start of the Road" is promoted for review instead of a full sync right away.
...
## New Features
* Added a new "Artists" review page for "End of the Road" where every artist's eligibility for the playlist can be viewed and toggled.
* Changing an artist's status there only affects future eligibility, not the playlist's current content.
...
## New Features
* New releases by artists already represented on "End of the Road" are now automatically staged as challengers on "Start of the Road", so they show up on the Challengers page without any manual step.
* Reissues, deluxe/live editions and already reviewed releases are not staged again.
...
## New Features
* Added a new "Challengers" page to review candidate tracks staged on "Start of the Road" against an artist's current track on "End of the Road".
* Each challenger can be accepted (replacing the artist's current track) or discarded (removing it from staging) with one click.
* A manual track swap done directly in Spotify is now recognized and tracked automatically on the next playlist sync.
...
## Bugfixes / Chore
* Reverted the weekly listening period back to Monday-Sunday, and the Slack digest is sent Monday again as before.
* The Slack weekly digest now shows the start date of the reported week.
