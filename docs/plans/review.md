...
* **`PlaylistSettingsResource.PlaylistRow` – date formatting duplicated inside a nested data class.** The computed property `lastSyncTimeFormatted` performs its own date-time formatting using `ZoneId.systemDefault()` and a `DateTimeFormatter`. This is inconsistent with `TemplateFormattingExtensions`, which centralizes all formatting logic and always formats in UTC. The `PlaylistRow` formatting logic should either use `TemplateFormattingExtensions` or be aligned to UTC.
